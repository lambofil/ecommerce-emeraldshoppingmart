...
import { increaseQuantity, decreaseQuantity, deleteProduct } from "@/redux/shoppingSlice";
        ...
        <div className="flex flex-col gap-y-2">
            ...
            productData?.map((item:Productss)=> (
                <div key={item._id} className="w-full bg-white p-4 flex flex-col md:flex-row items-center justify-between gap-4" >
                    <div className="flex items-center gap-x-3 w-full md-full md:w-1/3">
                        <span onClick={()=> dispatch(deleteProduct(item?._id))} className="text-lg hover:text-red-600 cursor-pointer duration-200"><AiOutlineClose /> </span>
                    ...
                    <div className="flex items-center justify-start gap-x-3 border-[1px] border-slate-300 py-2 px-4 w-full md:w-auto">
                        ...
                        <div className="flex items-center text-lg w-20 justify-between">
                            <span onClick={()=>dispatch(decreaseQuantity(item))} className="cursor-pointer">
                                <FiChevronLeft />
                            </span>
                            <span>
                                {item?.quantity}
                    ...
                    <div className="w-full md:w-1/3 flex items-end justify-start md:justify-end">
                        <p className="text-lg font-semibold"><FormattedPrice amount={item?.price * item?.quantity} /></p>
                    </div>
                </div>   
            ))}
